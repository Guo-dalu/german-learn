...
const jsonModules = import.meta.glob<ContentFile>('/content/vocabulary/*.json', { eager: true, import: 'default' })
...
export default function VocabularyIndex() {
  ...
  const topics = useMemo(() => {
    return Object.entries(jsonModules).map(([path, json]) => {
      const slug = path.replace('/content/vocabulary/', '').replace('.json', '')
      const raw = mdModules[`/content/vocabulary/${slug}.${lang}.md`] ?? mdModules[`/content/vocabulary/${slug}.md`] ?? ''
