...
**JSON exercises** — multilingual fields use `{ en, zh }` objects. Exercise types: `fill-in`, `multiple-choice`, `matching`. See `content/vocabulary/food.json` for a full example. Render multilingual fields using `lang` from i18n context.
...
- Each `.md`: 3–8 example sentences; each `.json`: 5–10 exercises
